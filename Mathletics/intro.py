...
class Introduction(Scene):
    ...
    def construct(self):
        ...
        ltext=Tex("launches", color=BLACK).next_to(kjssclogo1, DOWN).scale(1.5)
        self.wait(0.5)
        self.play(FadeIn(kjssclogo1))
        self.play(Write(ltext))
        self.wait(1)
        self.play(ReplacementTransform(kjssclogo1, kjssclogo2), Unwrite(ltext))
        # self.wait(0.5)
        self.play(FadeIn(logob))
        # self.play(Write(logos, fill_color=None))
        self.play(Write(logotext1.next_to(logob, DOWN, buff=0.2)))
        self.wait(1)

        slogan=Tex('''Let us take you to the beautiful depths of\n
truth, because Math is the only place where\n
truth and beauty mean the same thing.''', color=BLACK).next_to(logotext1, DOWN, buff=0.5)
        stext=Text("by", color=BLACK).move_to(DOWN*2).scale(0.5)
...
#         )
        ...
        mentionb=Tex("\\textbf{Honourable mention}", color=BLACK)
        mentions=Tex("\\textbf{Honourable mention}", color=BLACK).move_to(UP*2)
        self.play(Write(mentionb))
        self.wait()
        self.play(ReplacementTransform(mentionb, mentions))

        co_foundt=Tex("Co-founder", color=BLACK).move_to(UP*1)
        co_found1=Text("Hetvik Gada", color=BLACK).move_to(LEFT*3).scale(0.5)
        co_found2=Text("Mihir Myatra", color=BLACK).scale(0.5)#.move_to(UP*1)
        co_found3=Text("Garima Joshi", color=BLACK).move_to(RIGHT*3).scale(0.5)
        self.play(Write(co_foundt))
        self.play(Write(co_found1))
        self.play(Write(co_found2))
        self.play(Write(co_found3))
        self.wait()

        hod=Text("Mr. Subhash Krishnan", color=BLACK).scale(0.5).move_to(DOWN*2+LEFT*3.25)
        hodt=Tex("HOD of Department of Mathematics", color=BLACK).next_to(hod, DOWN).scale(0.6)
        principle=Text("Dr. Pradnya Prabhu", color=BLACK).scale(0.5).move_to(DOWN*2+RIGHT*3)
        principlet=Tex('''Principal\n
K. J. Somaiya College of Science and Commerce''', color=BLACK).next_to(principle, DOWN).scale(0.6)
        self.play(Write(hod))
        self.play(Write(hodt.next_to(principlet, LEFT, buff=0.8)))
        self.play(Write(principle))
        self.play(Write(principlet))
        self.wait(2)
