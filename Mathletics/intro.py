...
truth and beauty mean the same thing.''').next_to(logotext1, DOWN, buff=0.5)
        ...
        act1=Text("Continuous posting of mathematical wisdom").scale(0.31).next_to(dot1)
        self.play(Write(act1), run_time=0.75)
        self.wait(0.5)
        self.play(Create(dot2), run_time=0.5)
        act2=Text("Organizing weekly recreational activities in mathematics such as puzzles, riddles, catch-problems, number games etc.").scale(0.31).next_to(dot2)
        self.play(Write(act2), run_time=0.75)
        self.wait(0.5)
        self.play(Create(dot3), run_time=0.5)
        act3=Text("Organizing webinars by renowned mathematics scholar.").scale(0.31).next_to(dot3)
        self.play(Write(act3), run_time=0.75)
        self.wait(0.5)
        self.play(Create(dot4), run_time=0.5)
        act4=Text("Celebrating days and events pertaining to the history of mathematics or people of mathematics.").scale(0.31).next_to(dot4)
        self.play(Write(act4), run_time=0.75)
        self.wait(0.5)
        self.play(Create(dot5), run_time=0.5)
        act5=Text("Organizing competitions on interesting mathematical topics.").scale(0.31).next_to(dot5)
        self.play(Write(act5), run_time=0.75)
        self.wait(1)
        groupa1=VGroup(act1, act2, act3, act4, act5)
        self.play(Unwrite(groupa1))
        self.wait(0.5)
        act6=Text("Organizing mathematical exhibitions/fairs.").scale(0.31).next_to(dot1)
        self.play(Write(act6), run_time=0.75)
        self.wait(0.5)
        act7=Text("Organizing outings of mathematical interest such as visits to Mathematical institutions, Museums, Banks, Market Places etc.").scale(0.31).next_to(dot2)
        self.play(Write(act7), run_time=0.75)
        self.wait(0.5)
        act8=Text("Organizing events of watching mathematical documentaries, movies together, or even going to places close to nature.").scale(0.31).next_to(dot3)
        self.play(Write(act8), run_time=0.75)
        self.wait(0.5)
        act9=Text("Sharing mathematical stories, meaningful poem on mathematics, presentations on some topics by the members.").scale(0.31).next_to(dot4)
        self.play(Write(act9), run_time=0.75)
        self.wait(0.5)
        act10=Text("Discussions on real life applications of Mathematics.").scale(0.31).next_to(dot5)
        ...
        hod=Text("Mr. Subhash Krishnan").scale(0.5).move_to(DOWN*2+LEFT*3)
        hodt=Tex("HOD of Department of Mathematics").next_to(hod, DOWN).scale(0.5)
        principle=Text("Dr. Pradnya Prabhu").scale(0.5).move_to(DOWN*2+RIGHT*3)
        principlet=Tex('''Principal\n
K. J. Somaiya College of Science and Commerce''').next_to(principle, DOWN).scale(0.5)
        self.play(Write(hod), run_time=0.5)
        self.play(Write(hodt.next_to(principlet, LEFT, buff=1.4)), run_time=0.5)
